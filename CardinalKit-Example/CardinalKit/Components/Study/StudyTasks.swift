...
//
//        let ldp_countdownStep = ORKCountdownStep(identifier: "ldp_CountdownStep")
//
//        ldp_countdownStep.stepDuration = 5
//
//
//        steps += [ldp_countdownStep]
//
        
//        let lpd_recordingSettings = [
...
//
//
//        let ldp_config = ORKAudioRecorderConfiguration(identifier: "LPP_Recorder", recorderSettings: lpd_recordingSettings)
//
////
//        let ldp_audio = ORKAudioStep(identifier: "ldp_Audio")
//
//        ldp_audio.stepDuration = 6
//
//        ldp_audio.shouldContinueOnFinish = false;
//
//        ldp_audio.recorderConfigurations?.append(ldp_config)
////
        
        
        
        steps += LDP_audio.steps
        ...
        let l_abi = ORKAnswerFormat.decimalAnswerFormat(withUnit: "abi")
        
        let LDP_ABI = ORKQuestionStep(identifier: "LDP_ ABI", title: nil, question: "What is your ABI?", answer: l_abi)
...
//        let audio_step = ORKAudioStep(identifier: "Speech")
        let lpt_countdownStep = ORKCountdownStep(identifier: "lpt_CountdownStep")
        
        lpt_countdownStep.stepDuration = 5
        
        
        l_steps += [lpt_countdownStep]
        
        
        let lpt_recordingSettings = [
            AVFormatIDKey : UInt(kAudioFormatAppleLossless),
            AVNumberOfChannelsKey : 2,
            AVSampleRateKey: 44100.0
        ] as [String : Any]
        ...
        let lpt_config = ORKAudioRecorderConfiguration(identifier: "LPT_Recorder", recorderSettings: lpt_recordingSettings)
        
//
        let lpt_audio = ORKAudioStep(identifier: "lpt_Audio")
        
        lpt_audio.stepDuration = 6
        
        lpt_audio.shouldContinueOnFinish = false;
        
        lpt_audio.recorderConfigurations?.append(lpt_config)
      
        l_steps += [lpt_audio]
        
    
        let i_lpt = ORKImageCaptureStep(identifier: "LPT_Image")

        l_steps += [i_lpt]
        
        let ABI_lpt = ORKAnswerFormat.decimalAnswerFormat(withUnit: "abi")
        
        let LPT_ABI = ORKQuestionStep(identifier: "LPT_ABI", title: nil, question: "What is your ABI?", answer: ABI_lpt)
        
        l_steps += [LPT_ABI]
        
        
        return ORKOrderedTask(identifier: "LPT", steps: l_steps)
    ...
    static let RDP_audio: ORKOrderedTask = {
        
        return ORKOrderedTask.audioTask(withIdentifier: "RDP_Audio" , intendedUseDescription: "The doppler sound data is strong diagnostic tool for artery disease (RDP)" , speechInstruction: nil, shortSpeechInstruction: "Recording LPT audio...." , duration: 6, recordingSettings: nil, checkAudioLevel: true, options: [])
        
    }()
    
    
    
    static let RDP: ORKOrderedTask = {
        var rd_steps = [ORKStep]()
...
//
        ...
        let rdp_countdownStep = ORKCountdownStep(identifier: "rdp_CountdownStep")
        
        rdp_countdownStep.stepDuration = 5
        
        
        let rdp_recordingSettings = [
            AVFormatIDKey : UInt(kAudioFormatAppleLossless),
            AVNumberOfChannelsKey : 2,
            AVSampleRateKey: 44100.0
        ] as [String : Any]
        ...
        let rdp_config = ORKAudioRecorderConfiguration(identifier: "RDP_Recorder", recorderSettings: rdp_recordingSettings)
        
//
        let rdp_audio = ORKAudioStep(identifier: "rdp_Audio")
        
        rdp_audio.stepDuration = 6
        
        rdp_audio.shouldContinueOnFinish = false;
        
        rdp_audio.recorderConfigurations?.append(rdp_config)
      
        
        rd_steps += [rdp_countdownStep]
        rd_steps += [rdp_audio]
    
        let i_rdp = ORKImageCaptureStep(identifier: "RDP_Image")

        rd_steps += [i_rdp]
        
        let ABI_rdp = ORKAnswerFormat.decimalAnswerFormat(withUnit: "abi")
        
        let RDP_ABI = ORKQuestionStep(identifier: "RDP_ABI", title: nil, question: "What is your ABI?", answer: ABI_rdp)
        
        rd_steps += [RDP_ABI]
        
        
        return ORKOrderedTask(identifier: "RDP", steps: rd_steps)
        
    }()
    
    
    static let RPT: ORKOrderedTask = {
        var rpt_steps = [ORKStep]()
        
        
//        let audio_step = ORKAudioStep(identifier: "Speech")
        let rpt_countdownStep = ORKCountdownStep(identifier: "rpt_CountdownStep")
        
        rpt_countdownStep.stepDuration = 5
        
        rpt_steps += [rpt_countdownStep]
        
        
        let rpt_recordingSettings = [
            AVFormatIDKey : UInt(kAudioFormatAppleLossless),
            AVNumberOfChannelsKey : 2,
            AVSampleRateKey: 44100.0
        ] as [String : Any]
        ...
        let rpt_config = ORKAudioRecorderConfiguration(identifier: "RPT_Recorder", recorderSettings: rpt_recordingSettings)
        
//
        let rpt_audio = ORKAudioStep(identifier: "rpt_Audio")
        
        rpt_audio.stepDuration = 6
        
        rpt_audio.shouldContinueOnFinish = false;
        
        rpt_audio.recorderConfigurations?.append(rpt_config)
        
        rpt_steps += [rpt_audio]
        
    
        let i_rpt = ORKImageCaptureStep(identifier: "RPT_Image")

        rpt_steps += [i_rpt]
        
        let ABI_rpt = ORKAnswerFormat.decimalAnswerFormat(withUnit: "abi")
        
        let RPT_ABI = ORKQuestionStep(identifier: "RPT_ABI", title: nil, question: "What is your ABI?", answer: ABI_rpt)
        
        rpt_steps += [RPT_ABI]
        
        
        return ORKOrderedTask(identifier: "RPT", steps: rpt_steps)
    ...
    static let FinalTask: ORKOrderedTask = {
        ...
        dorsalis_pedis.text = "Please be ready to analyze the doppler sound, image of the waveform, and ankle brachial index for the doralist pedis on your left leg.THE AUDIO WILL START IMMEDIATELY"
...
//
        ...
        posterior_tibial.text = "Please be ready to analyze the doppler sound, image of the waveform, and ankle brachial index for the posterior tibial on your left leg.THE AUDIO WILL START IMMEDIATELY"
        ...
        dorsalis_pedis_r.title = "DP Right Leg"
        dorsalis_pedis_r.text = "Please be ready to analyze the doppler sound, image of the waveform, and ankle brachial index for the doralist pedis on your right leg. THE AUDIO WILL START IMMEDIATELY"
        ...
        posterior_tibial_r.text = "Please be ready to analyze the doppler sound, image of the waveform, and ankle brachial index for the posterior tibial on your right leg.THE AUDIO WILL START IMMEDIATELY"
        ...
        let ans = ORKNavigableOrderedTask(identifier:"ans", steps: steps)
        
        
        let predicate = ORKResultPredicate.predicateForBooleanQuestionResult(with:
          ORKResultSelector(resultIdentifier: r.identifier), expectedAnswer: false)
        
        let rule = ORKPredicateStepNavigationRule(
          resultPredicatesAndDestinationStepIdentifiers: [(predicate, h.identifier)])
        ans.setNavigationRule(rule, forTriggerStepIdentifier: r.identifier)
